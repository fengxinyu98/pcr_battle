...
class UIMatcher:
    ...
    def findpic(screen, template_paths=['img/tiaoguo.jpg']):
        ...
        for template_path in template_paths:
            ...
            '''
            if max_val > 0.8:
                cv2.rectangle(screen_show, (int(max_loc[0]), int(max_loc[1])), (int(
                    max_loc[0]+w), int(max_loc[1]+h)), (0, 0, 255), 2)
    ...
    def find_gaoliang(screen):
        ...
        index_1 = np.mean(np.argwhere(
            binary[63:, :] == 255), axis=0).astype(int)
